...
func TestGOST_Suites_Registered(t *testing.T) {
	...
	t.Run("GOST2012_alias_C102", func(t *testing.T) {
		...
		if s.KX != suites.KexGOST2012_256 {
	...
	t.Run("GOST2012_Kuznyechik_C100", func(t *testing.T) {
		...
		if s.KX != suites.KexGOST2018_256 {
...
func TestGOST_PRF_Streebog256(t *testing.T) {
	...
	out1, err := suites.PRF(s.PRF.Hash, secret, label, seed, 32)
	if err != nil {
		t.Fatalf("PRF error: %v", err)
	}

	out2, err := suites.PRF(s.PRF.Hash, secret, label, seed, 32)
	if err != nil {
		t.Fatalf("PRF error: %v", err)
	}
...
func TestGOST_PRF_R341194(t *testing.T) {
	...
	out1, err := suites.PRF(s.PRF.Hash, secret, label, seed, 48)
	if err != nil {
		t.Fatalf("PRF error: %v", err)
	}

	out2, err := suites.PRF(s.PRF.Hash, secret, label, seed, 48)
	if err != nil {
		t.Fatalf("PRF error: %v", err)
	}
