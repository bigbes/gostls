...
func TestKuznyechikCTROMACProtector_EngineEtalons(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wantTail: mustHexCTROMAC(t, "0a81299b3598195dd45168a63850a76e1a4f1e6dd5ef72593fae765571ec37e717f5b86285bb5bfd83b66ab763865208"),
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantTail != nil && !bytes.Equal(sealed[len(sealed)-len(tc.wantTail):], tc.wantTail) {
				t.Fatalf("sealed tail mismatch:\n got  %x\n want %x", sealed[len(sealed)-len(tc.wantTail):], tc.wantTail)
			}
		})
	}
}
